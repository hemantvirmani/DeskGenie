...
2. Download `DeskGenie-vX.X.X-windows.zip` (ignore the "Source code" archives — those are auto-generated by GitHub)
3. Extract the zip anywhere
4. Rename `config.json.example` to `config.json` and open it
5. Set `llm.activeProvider` and add your API key for that provider (see table below)
6. Run `DeskGenie.exe`
...
> **Tip:** You can also place `config.json` at `%LOCALAPPDATA%\DeskGenie\config.json` to keep it separate from the app folder. If both exist, the `%LOCALAPPDATA%` one takes priority.
